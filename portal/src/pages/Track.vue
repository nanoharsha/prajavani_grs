<template>
  <div class="max-w-2xl mx-auto px-4 py-8">
    ...
    <div class="card p-4 mb-6">
      <div class="flex gap-2">
        <input v-model="query" type="text"
          placeholder="Registration number or 10-digit mobile"
          class="input flex-1" @keyup.enter="load" />
        <button class="btn-primary" :disabled="loading || !query.trim()" @click="load">
    ...
    <!-- Mobile search: list of complaints -->
    <div v-else-if="list.length" class="space-y-3">
      <p class="text-sm text-gray-500 font-medium px-1">{{ list.length }} complaint{{ list.length > 1 ? 's' : '' }} found for this mobile number:</p>
      <div
        v-for="g in list" :key="g.registration_no"
        class="card p-4 flex items-center justify-between gap-3 cursor-pointer hover:shadow-md transition-shadow"
        @click="loadByRegNo(g.registration_no)"
      >
        <div>
          <p class="font-mono text-xs text-gray-400 mb-0.5">{{ g.registration_no }}</p>
          <p class="font-semibold text-gray-800 text-sm">{{ g.department }} — {{ g.category }}</p>
          <p class="text-xs text-gray-500 mt-0.5">Filed {{ daysAgo(g.filing_date) }}</p>
        </div>
        <span :class="badgeClass(g.status_label)">{{ g.status_label }}</span>
      </div>
    </div>
    ...
    <template v-else-if="data">
      ...
      <!-- Back to list button (shown when drilled in from a mobile search) -->
      <button v-if="prevList.length" class="text-sm text-brand-600 mb-4 flex items-center gap-1" @click="backToList">
        ← Back to list
      </button>
    ...
    <div v-else-if="!loading" class="text-center text-gray-400 py-16">
      ...
      <p class="font-medium text-gray-500">Enter your registration number or mobile number above.</p>
    </div>
...
const query   = ref('')
const data    = ref(null)
const list    = ref([])     // mobile-search results
const prevList = ref([])    // saved list when drilling into a single complaint
const error   = ref('')
const loading = ref(false)

onMounted(() => {
  if (route.query.reg) {
    query.value = route.query.reg
    load()
  }
})
...
async function load() {
  ...
  prevList.value = []
  try {
    const res = await api.trackGrievance(q)
    if (res?.error) {
      error.value = res.error
    } else if (res?.grievances) {
      // Mobile search returned a list
      list.value = res.grievances
    } else {
...
async function loadByRegNo(regNo) {
  prevList.value = list.value
  list.value = []
  loading.value = true
  error.value   = ''
  try {
    const res = await api.trackGrievance(regNo)
...
function badgeClass(label) {
  return `text-xs font-semibold px-3 py-1.5 rounded-full flex-shrink-0 ${STATUS_COLORS[label] || 'bg-gray-100 text-gray-600'}`
}
